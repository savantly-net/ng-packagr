...
export const stylesheetTransform: Transform = pipe(
  map(graph => {
    // TODO...
    console.log('Render stylesheets...');
...
async function renderPostCss(filePath: string, cssStyles: string, cssUrl: CssUrl): Promise<string> {
  ...
  log.debug(`postcss with autoprefixer for ${filePath}`);
  ...
  postCssPlugins.push(postcssComments({ removeAll: true }));
